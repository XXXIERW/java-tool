...
public class PropertiesLoader {
	...
	public void storeProperties(Properties properties, String... resourcesPaths) {
		for (String location : resourcesPaths) {

			logger.debug("Storing properties file from path:{}", location);

			OutputStream fos = null;
			try {
				File file = resourceLoader.getResource(location).getFile();
				fos = new FileOutputStream(file);
				properties.store(fos, "Application environment");
			} catch (IOException ex) {
				logger.info("Could not store properties from path:{}, {} ", location, ex.getMessage());
			} finally {
