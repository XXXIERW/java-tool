...
public class NetUtils {
    ...
    public static final String LOCALHOST = "127.0.0.1"; // 默认本机地址
    ...
    /**
     * <pre>
     * 查找策略：首先看是否已经查到ip --> 轮询网卡 --> hostname对应的ip --> 根据连接目标端口得到的本地ip
     * </pre>
     * 
     * @return local ip
     */
    public static InetAddress getLocalAddress(Map<String, Integer> destHostPorts) {
        ...
        InetAddress localAddress = getLocalAddressByNetworkInterface();
    ...
    private static InetAddress getLocalAddressBySocket(Map<String, Integer> destHostPorts) {
        ...
        for (Map.Entry<String, Integer> entry : destHostPorts.entrySet()) {
            ...
            try {
                ...
                logger.warn(String.format("Failed to retrieving local address by connecting to dest host:port(%s:%s) false, e=%s", host,
                        port, e));
            }
        }
        return null;
