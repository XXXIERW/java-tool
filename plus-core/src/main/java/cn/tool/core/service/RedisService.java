...
public class RedisService {
    ...
    public boolean exists(final String key) {
        Jedis jedis = null;
        try {
            jedis = pool.getResource(); // 查看连接池是否已经连接
    ...
    public <T> T getJSONToObject(Class<?> clazz, String key) {
        ...
        try {
            ...
            return (T) JsonUtils.stringToObject(value, clazz);
        } finally {
            if (jedis != null) {
                jedis.close();
            }
    ...
    public <T> List<T> mget(Class<?> clazz, final String... keys) {
        ...
        try {
            ...
            for (String value : values) {
                list.add((T) JsonUtils.stringToObject(value, clazz));
            }
            return list;
        } finally {
            if (jedis != null) {
    ...
    public <T> String mset(final Map<String, T> map) {
        ...
        try {
            ...
            for (String key : map.keySet()) {
                keyValues[2 * i] = key;
                keyValues[2 * i + 1] = JsonUtils.objectToString(map.get(key));
                i++;
            }
